  ...
  const fetchData = async () => {
    try {
      ...
      const enrichedTrainings = trainingsData.map(t => {
        ...
        const absent = tAttends.filter(a => a.status === 'absent').length;
    ...
    <>
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Monitor attendance rates across all training sessions</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {trainings.map((training, index) => (
              <Card key={training.id} className="flex flex-col shadow-sm border-none ring-1 ring-slate-100 hover:shadow-md transition-all duration-200 group">
                <CardHeader className="pb-4 border-b border-slate-50 bg-slate-50/50">
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      <span className="text-xs font-bold text-primary uppercase tracking-wider mb-1 block">Day {index + 1}</span>
                      <CardTitle className="text-lg line-clamp-1 text-slate-900" title={training.title}>
                        {training.title}
                      </CardTitle>
                    </div>
                    <Badge variant={training.status === 'completed' ? 'outline' : 'default'} className={training.status === 'completed' ? 'bg-slate-100 text-slate-600 border-none' : ''}>
                      {training.status}
                    </Badge>
                  </div>
                  <div className="flex items-center gap-4 text-sm text-slate-500">
                    <span className="flex items-center gap-1.5"><Calendar className="h-4 w-4 text-slate-400" /> {training.date}</span>
                    <span className="flex items-center gap-1.5"><Users className="h-4 w-4 text-slate-400" /> {training.stats.total} Assigned</span>
                  </div>
                </CardHeader>
                <CardContent className="flex-1 flex flex-col justify-between pt-6">
                  <div className="mb-6">
                    ...
                    <Progress value={training.stats.rate} className="h-2.5 mb-4 bg-slate-100" indicatorClassName="bg-primary" />
                    
                    <div className="grid grid-cols-2 gap-2">
                      <div className="flex items-center gap-2 bg-emerald-50 px-3 py-2 rounded-lg">
                        <CheckCircle2 className="h-4 w-4 text-emerald-600" />
                        <span className="text-sm font-medium text-emerald-700">{training.stats.present} Present</span>
                      </div>
                      <div className="flex items-center gap-2 bg-rose-50 px-3 py-2 rounded-lg">
                        <XCircle className="h-4 w-4 text-rose-600" />
                        <span className="text-sm font-medium text-rose-700">{training.stats.absent} Absent</span>
                      </div>
                    </div>
                  </div>
                  <Button asChild variant="outline" className="w-full mt-auto group-hover:bg-primary group-hover:text-white group-hover:border-primary transition-colors">
